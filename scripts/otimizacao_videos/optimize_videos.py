...
#
#   Otimizar só alguns vídeos específicos (passa o caminho de cada um):
#     python3 scripts/otimizacao_videos/optimize_videos.py topicos/curiosidades/autorais/videos/elden-ring-1.webm topicos/curiosidades/autorais/videos/god-of-war.webm
#
...
def main():
    ...
    if args:
        # Modo específico: só otimiza os arquivos passados na linha de comando.
        video_files = []
        for path in args:
            ...
            if os.path.splitext(path)[1].lower() not in VIDEO_EXTS:
                print(f"AVISO: não é .mp4/.webm, pulando: {path}")
                continue
            video_files.append(path)
        print(f"Otimizando {len(video_files)} vídeo(s) específico(s).\n")
    else:
        # Modo padrão: varre o repositório inteiro atrás de vídeos.
        print("Nenhum arquivo específico passado — procurando vídeos a partir da pasta atual...")
        video_files = []
