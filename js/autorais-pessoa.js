...
function iniciar() {
  ...
  const pararPessoa = onSnapshot(pessoaRef, (snapshot) => {
    if (!snapshot.exists()) {
  ...
  const pararConteudos = onSnapshot(conteudosRef, (snapshot) => {
    const docs = snapshot.docs.sort((a, b) => obterMillis(a.data().criadoEm) - obterMillis(b.data().criadoEm));
    renderizarConteudos(docs, pesquisador);
  }, (erro) => {
  ...
  el.btnExcluir.addEventListener('click', async () => {
    if (!pesquisador || !dadosPessoa) return;
    const confirmar = window.confirm(`Remover ${dadosPessoa.nome} e todos os conteúdos cadastrados pelo sistema?`);
    if (!confirmar) return;
  ...
  function tentarAbrirNovo() {
    ...
    abriuNovo = true;
    history.replaceState({}, '', `/topicos/autorais/pessoa.html?secao=${encodeURIComponent(secao)}&id=${encodeURIComponent(pessoaId)}`);
...
function renderizarConteudos(docs, pesquisador) {
  if (!docs.length) {
    const aviso = criarAviso('Ainda não há conteúdo publicado por esta pessoa.');
    el.conteudos.replaceChildren(aviso);
    return;
  }

  const fragmento = document.createDocumentFragment();
  docs.forEach((item) => fragmento.appendChild(criarConteudo(item.id, item.data(), pesquisador)));
...
function criarConteudo(id, dados, pesquisador) {
  ...
  if (pesquisador) {
    ...
    excluir.addEventListener('click', async () => {
      ...
      try {
        await apagarArquivosDoConteudo(dados);
        await deleteDoc(doc(db, 'participantesAutorais', pessoaId, 'conteudos', id));
      } catch (erro) {
  ...
  if (secao === 'desenhos') {
    ...
    if (dados.descricao) artigo.appendChild(paragrafoDescricao(dados.descricao));
  }
...
function abrirModalConteudo(dadosPessoa) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const conteudoRef = doc(collection(db, 'participantesAutorais', pessoaId, 'conteudos'));
    const dados = {
    ...
    try {
      ...
      if (secao === 'desenhos') {
        ...
        const caminho = caminhoConteudo(conteudoRef.id, 'imagem', arquivo.name);
        dados.imagemPath = caminho;
        dados.imagemUrl = await enviarComMensagem(arquivo, caminho, msg, barraSpan, 'Enviando desenho');
      ...
      if (secao === 'musica') {
        dados.tipo = 'musica';
        dados.descricao = form.elements.descricao.value.trim();
        const arquivo = form.elements.video.files[0];
        const caminho = caminhoConteudo(conteudoRef.id, 'video', arquivo.name);
        dados.videoPath = caminho;
        dados.videoUrl = await enviarComMensagem(arquivo, caminho, msg, barraSpan, 'Enviando vídeo');
        pathsCriados.push(caminho);
      }
      ...
      if (secao === 'curiosidades') {
        ...
        dados.descricao = form.elements.descricao.value.trim();
        const imagem = form.elements.imagem.files?.[0];
        const video = form.elements.video.files?.[0];

        if (imagem) {
          const caminho = caminhoConteudo(conteudoRef.id, 'imagem', imagem.name);
          dados.imagemPath = caminho;
          dados.imagemUrl = await enviarComMensagem(imagem, caminho, msg, barraSpan, 'Enviando imagem');
          pathsCriados.push(caminho);
        }

        if (video) {
          const caminho = caminhoConteudo(conteudoRef.id, 'video', video.name);
          dados.videoPath = caminho;
          dados.videoUrl = await enviarComMensagem(video, caminho, msg, barraSpan, 'Enviando vídeo');
          pathsCriados.push(caminho);
        }
      }
...
function abrirModalEditarPessoa(dadosPessoa, pessoaRef) {
  ...
  modal.innerHTML = `
    <div class="et-modal__caixa" role="dialog" aria-modal="true" aria-labelledby="et-editar-pessoa-titulo">
      ...
      <form data-form novalidate>
        <div class="et-campo">
          <label for="et-editar-nome">Nome</label>
          <input id="et-editar-nome" name="nome" type="text" maxlength="120" required>
        </div>
        <div class="et-campo">
          <label for="et-editar-descricao">Descrição</label>
          <textarea id="et-editar-descricao" name="descricao" maxlength="1200" required></textarea>
        </div>
        <div class="et-campo et-arquivo">
          <label for="et-editar-foto">Trocar imagem (opcional)</label>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const novaFoto = form.elements.foto.files?.[0];

    if (!nome || !descricao) {
      erroMsg(msg, 'Preencha nome e descrição.');
      return;
    }
    if (novaFoto && (!novaFoto.type.startsWith('image/') || novaFoto.size > 15 * 1024 * 1024)) {
    ...
    const atualizacao = { nome, descricao, atualizadoEm: serverTimestamp() };
    let novoPath = null;
    ...
    try {
      if (novaFoto) {
        ...
        atualizacao.fotoUrl = await enviarComMensagem(novaFoto, novoPath, msg, barraSpan, 'Enviando nova foto');
      }

      await updateDoc(pessoaRef, atualizacao);
      if (novaFoto && dadosPessoa.fotoPath && dadosPessoa.fotoPath !== novoPath) {
        await deleteObject(ref(storage, dadosPessoa.fotoPath)).catch(() => {});
      }
      salvando = false;
      fechar();
      ...
      if (novoPath) await deleteObject(ref(storage, novoPath)).catch(() => {});
      erroMsg(msg, mensagemErroFirebase(erro));
      salvando = false;
      salvar.disabled = false;
...
function camposConteudoHtml(dadosPessoa) {
  ...
  const comuns = `
    <div class="et-campo">
      <label for="et-conteudo-titulo">Nome / título</label>
      <input id="et-conteudo-titulo" name="titulo" type="text" maxlength="160" required>
    </div>
    <div class="et-campo">
      <label for="et-conteudo-autor">Quem criou / escreveu</label>
      <input id="et-conteudo-autor" name="autor" type="text" maxlength="160" value="${nome}" required>
    </div>
  `;
  ...
  if (secao === 'poemas') {
    return `${comuns}
      <div class="et-campo">
        <label for="et-conteudo-texto">Poema</label>
        <textarea id="et-conteudo-texto" name="texto" rows="12" maxlength="20000" required></textarea>
      </div>`;
  }
  ...
  if (secao === 'desenhos') {
    return `${comuns}
      <div class="et-campo">
        <label for="et-conteudo-descricao">Descrição (opcional)</label>
        <textarea id="et-conteudo-descricao" name="descricao" rows="4" maxlength="2500"></textarea>
      </div>
      <div class="et-campo et-arquivo">
        <label for="et-conteudo-imagem">Imagem do desenho</label>
        <input id="et-conteudo-imagem" name="imagem" type="file" accept="image/*" required>
      </div>`;
  }
  ...
  if (secao === 'musica') {
    return `${comuns}
      <div class="et-campo">
        <label for="et-conteudo-descricao">Descrição da música / apresentação</label>
        <textarea id="et-conteudo-descricao" name="descricao" rows="5" maxlength="3500" required></textarea>
      </div>
      <div class="et-campo et-arquivo">
        <label for="et-conteudo-video">Vídeo da música</label>
        <input id="et-conteudo-video" name="video" type="file" accept="video/*" required>
      </div>`;
  }

  return `${comuns}
    <div class="et-campo">
      <label for="et-conteudo-descricao">Texto / descrição da curiosidade</label>
      <textarea id="et-conteudo-descricao" name="descricao" rows="7" maxlength="12000"></textarea>
    </div>
    <div class="et-campo et-arquivo">
      <label for="et-conteudo-imagem">Imagem (opcional)</label>
      <input id="et-conteudo-imagem" name="imagem" type="file" accept="image/*">
    </div>
    <div class="et-campo et-arquivo">
      <label for="et-conteudo-video">Vídeo (opcional)</label>
...
function validarConteudo(form) {
  ...
  if (!titulo || !autor) return { ok: false, mensagem: 'Preencha o título e o autor.' };

  if (secao === 'poemas') {
    if (!form.elements.texto.value.trim()) return { ok: false, mensagem: 'Digite o poema.' };
  }

  if (secao === 'desenhos') {
    const imagem = form.elements.imagem.files?.[0];
    if (!imagem) return { ok: false, mensagem: 'Escolha a imagem do desenho.' };
  ...
  if (secao === 'musica') {
    const video = form.elements.video.files?.[0];
    if (!form.elements.descricao.value.trim()) return { ok: false, mensagem: 'Preencha a descrição da música.' };
    if (!video) return { ok: false, mensagem: 'Escolha o vídeo da música.' };
  ...
  if (secao === 'curiosidades') {
    ...
    const imagem = form.elements.imagem.files?.[0];
    const video = form.elements.video.files?.[0];
    if (!descricao && !imagem && !video) {
      return { ok: false, mensagem: 'Adicione texto, imagem, vídeo ou uma combinação deles.' };
    }
    if (imagem) {
      const erro = validarImagem(imagem);
      if (erro) return { ok: false, mensagem: erro };
...
function validarImagem(arquivo) {
  ...
  if (arquivo.size > 15 * 1024 * 1024) return 'A imagem precisa ter no máximo 15 MB.';
  return '';
}

function validarVideo(arquivo) {
  if (!arquivo.type.startsWith('video/')) return 'Escolha um vídeo válido.';
  if (arquivo.size > 100 * 1024 * 1024) return 'O vídeo precisa ter no máximo 100 MB.';
  return '';
}
...
function enviarArquivo(arquivo, caminho, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    tarefa.on('state_changed', (snapshot) => {
      const percentual = snapshot.totalBytes ? snapshot.bytesTransferred / snapshot.totalBytes * 100 : 0;
...
function obterMillis(timestamp) {
  try { return timestamp?.toMillis?.() || 0; } catch { return 0; }
}
...
function mensagemErroFirebase(erro) {
  ...
  if (codigo.includes('unauthorized') || codigo.includes('permission-denied')) {
    return 'Sem permissão para salvar. Confira as regras do Firebase e sua sessão de pesquisador.';
  }
